...
from schema import AddComment, Comment, GraduatesWithComments, PaginatedGraduatesWithComments, ReturnComment, ReturnGraduate
...
@router.post("/add-picture", status_code=status.HTTP_201_CREATED, response_model=ReturnGraduate, summary="This route is used to add a new picture of a graduate.", description="It allows for only one picture to be added ata time.")
async def upload_graduation_picture(graduate_name: str, graduate_picture: UploadFile, api_access_code: str = Query(None, description="Special Key needed in order to have access to use the API"), db: Session = Depends(get_db)):
    graduate = {}
    ...
    if resp == "Fail-D":
        raise HTTPException(status_code= status.HTTP_400_BAD_REQUEST, detail="Could not add Graduate Picture to Database")
    if resp == "Fail-A":
        raise HTTPException(status_code= status.HTTP_400_BAD_REQUEST, detail="Could not add Graduate Picture to AWS Bucket")
    return resp

@router.post("/add-comment", status_code=status.HTTP_201_CREATED, response_model=ReturnComment, summary="This route enables new comments to be added to any picture of a graduate.", description="It returns the comment that was just sent as a response.")
def comment_on_a_picture(comment: AddComment, api_access_code: str = Query(None, description="Special Key needed in order to have access to use the API"), db: Session = Depends(get_db)):
    resp = UserCrud.check_secret_key(db, api_access_code, SECRET_KEY_ACCESS)
    if resp == "Fail":
        raise HTTPException(status_code= status.HTTP_401_UNAUTHORIZED, detail="You are not authorized")
...
def get_all_graduate_pictures_with_comments_preview_paginated_view(page:Optional[int] = None, api_access_code: str = Query(None, description="Special Key needed in order to have access to use the API"), db: Session = Depends(get_db)):
    ...
    if resp == "Fail":
        raise HTTPException(status_code= status.HTTP_401_UNAUTHORIZED, detail="You are not authorized")
    pictures = UserCrud.get_graduates_with_comments_paginated(api_access_code, db, items_per_page=10, page=page)
    return pictures

@router.get("/get-comments", status_code=status.HTTP_200_OK, response_model=Optional[List[Comment]], summary="This route is used to get all the comments under a specific picture of a graduate.", description="It returns a List of Json data.")
def get_comments_for_a_picture(graduate_id: UUID, api_access_code: str = Query(None, description="Special Key needed in order to have access to use the API"), db: Session = Depends(get_db)):
    resp = UserCrud.check_secret_key(db, api_access_code, SECRET_KEY_ACCESS)
    if resp == "Fail":
        raise HTTPException(status_code= status.HTTP_401_UNAUTHORIZED, detail="You are not authorized")
